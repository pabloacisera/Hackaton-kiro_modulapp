...
describe('NotificationsService', () => {
  ...
  it('unit.notification.createIncludesCorrectFields — notifyAdmins creates with all fields', async () => {
    const n = await service.notifyAdmins(
      'new_purchase', 'New order received', '/admin/orders/ord-1');
  ...
  it('unit.notification.markRead.propagatesToAllTabs — markRead updates in-memory state', async () => {
  ...
  it('broadcasts notification via registered broadcast function', async () => {
    ...
    const n = await service.notifyAdmins('new_complaint', 'Complaint received', '/admin/complaints/c-1');
