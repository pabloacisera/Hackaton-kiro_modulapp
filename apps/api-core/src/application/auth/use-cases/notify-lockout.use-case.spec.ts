...
describe('NotifyLockoutUseCase', () => {
  ...
  it('unit.notify-lockout.sendsWebSocketNotification — calls notify with email and userId', async () => {
    await useCase.execute('admin@modulapp.com', 'admin-uuid-123');
    expect(notificationService.notify).toHaveBeenCalledWith('admin@modulapp.com', 'admin-uuid-123');
  });
  ...
  it('unit.notify-lockout.persistsNotification — does not throw on success', async () => {
    await expect(useCase.execute('admin@modulapp.com', 'admin-uuid-123')).resolves.toBeUndefined();
  });
});
