...
- [x] TASK-catalog-1: Database migration + `Prototype` domain entity with invariants
...
- [x] TASK-catalog-2: `GET /catalog/prototypes` and `GET /catalog/prototypes/:id` endpoints
...
- [x] TASK-catalog-3: Redis cache for listings with invalidation on update
...
- [x] TASK-catalog-4: SSE endpoint `/catalog/stream` + event publisher from admin catalog module
...
- [x] TASK-catalog-5: `models/catalogApi.ts` + `controllers/useCatalog.ts` in `apps/landing`
...
- [x] TASK-catalog-6: `CatalogGrid` + `PrototypeCard` views
...
- [x] TASK-catalog-7: `PrototypeDetail` view + image gallery
...
- [x] TASK-catalog-8: `CatalogFilters` view + deactivation notice
