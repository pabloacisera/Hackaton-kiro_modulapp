...
- [ ] TASK-directpurchase-2: `POST /orders` endpoint with server-side price/stock re-read and `build_on_demand` validation
  ...
  - Deliverable: `apps/api-core/src/modules/orders/orders.controller.ts`, `apps/api-core/src/modules/orders/orders.service.ts`
  - Depends on: TASK-directpurchase-1, TASK-catalog-2
  - Assigned to: unassigned
  - Done criteria: `POST /orders` reads product price and stock from DB (not from request body). Returns 400 with message if stock=0 and `build_on_demand=false`. Validates customer email format (regex), returns 400 if invalid. Creates order with status `created`. Unit tests in `apps/api-core/src/modules/orders/**/*.spec.ts` verify: price re-read ignores client payload, stock=0 + build_on_demand=false blocks, email validation rejects invalid format, valid request creates order. All tests pass.
...
- [ ] TASK-directpurchase-3: HTTP client to `payment-service` to initiate payment
  ...
  - Deliverable: `apps/api-core/src/modules/orders/payment-client.ts`
...
- [ ] TASK-directpurchase-4: Webhook endpoint `POST /orders/webhooks/payment-result`
  ...
  - Deliverable: `apps/api-core/src/modules/orders/orders.controller.ts` (webhook route), `apps/api-core/src/modules/orders/webhook.validator.ts`
...
- [ ] TASK-directpurchase-5: Payment confirmation email to customer
  ...
  - Deliverable: `apps/api-core/src/modules/orders/order-email.service.ts`
...
- [ ] TASK-directpurchase-6: WebSocket notification to admin for new pending purchase
  ...
  - Deliverable: `apps/api-core/src/modules/orders/order-notification.service.ts`
...
- [ ] TASK-directpurchase-7: `PATCH /orders/:id/accept` endpoint (deducts stock, sets ETA)
  ...
  - Deliverable: `apps/api-core/src/modules/orders/orders.controller.ts` (accept route), `apps/api-core/src/modules/orders/orders.service.ts` (accept logic)
...
- [ ] TASK-directpurchase-8: `PATCH /orders/:id/reject` endpoint (mandatory reason + automatic refund)
  ...
  - Deliverable: `apps/api-core/src/modules/orders/orders.controller.ts` (reject route), `apps/api-core/src/modules/orders/orders.service.ts` (reject logic), `apps/api-core/src/modules/orders/refund-client.ts`
  - Depends on: TASK-directpurchase-5
  - Assigned to: unassigned
...
- [ ] TASK-directpurchase-9: BullMQ reconciliation job for hung payments
  ...
  - Deliverable: `apps/api-core/src/modules/orders/reconciliation.job.ts`
  - Depends on: TASK-directpurchase-3
  - Assigned to: unassigned
...
- [ ] TASK-directpurchase-10: Admin listing `GET /orders` with filters/search/pagination
  ...
  - Deliverable: `apps/api-core/src/modules/orders/orders.controller.ts` (list route), `apps/api-core/src/modules/orders/orders.service.ts` (list logic)
...
- [ ] TASK-directpurchase-13: Integration tests for full direct purchase flow
  ...
  - Deliverable: `apps/api-core/src/modules/orders/**/*.integration-spec.ts`
