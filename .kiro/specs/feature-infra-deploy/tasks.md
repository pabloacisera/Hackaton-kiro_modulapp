...
- [ ] TASK-infra-1: Setup Turborepo at monorepo root
  - Context: FR1 requires a monorepo that builds `api-core`, `landing`, and `admin-dashboard` via Turborepo. FR2+FR3 imply these apps are orchestrated together. Turborepo provides the build cache that CI (TASK-infra-6) must leverage.
  - Deliverable: `turbo.json`, workspace config in root `package.json` (glob `apps/*`, `services/api-core`)
  - Depends on: none
  - Assigned to: unassigned
  - Done criteria: `turbo run build` from root builds `apps/landing`, `apps/admin-dashboard`, and `services/api-core`; second run hits cache and completes in <5s; `turbo run lint` and `turbo run test` propagate to all workspaces.

- [ ] TASK-infra-2: Multi-stage Dockerfiles for all services
...
- [ ] TASK-infra-3: Nginx reverse proxy and statics configuration
...
- [ ] TASK-infra-4: Docker Compose orchestration and environment template
...
- [ ] TASK-infra-5: Database seed script with fake data
  ...
  - Depends on: TASK-infra-1 (needs working build), migrations from all domain features
  - Assigned to: unassigned
...
- [ ] TASK-infra-6: Extend CI pipeline with Docker build and Maven job
...
- [ ] TASK-infra-7: Per-environment deployment documentation and production script
...
- [ ] TASK-infra-8: End-to-end integration test of docker-compose environment
  ...
  - Depends on: TASK-infra-1, TASK-infra-2, TASK-infra-3, TASK-infra-4, TASK-infra-5
  - Assigned to: unassigned
